...
export class ModuleGenerator {
  ...
  private emitSubmodule(target: ConstructsMakerModuleTarget) {
    ...
    this.code.openBlock(
      `export class ${baseName} extends cdktf.TerraformModule`
    );
    ...
    this.code.line(`...this.inputs,`);

    for (const input of spec.inputs) {
      if (parseTypeInformation(input.type)?.kind === "set") {
        this.code.line(
          `${toCamelCase(input.name)}: "set(" + this.${toCamelCase(
            input.name
          )} + ") ",`
        );
      }
...
function parseTypeInformation(type: string) {
...
function parseComplexType(type: string): string | undefined {
  const typeInfo = parseTypeInformation(type);
  if (!typeInfo) {
    return undefined;
  }
  const { kind, innerType } = typeInfo;
  ...
  // We treat sets internally as arrays and convert them to sets on the way to terraform
  if (kind === "set") {
    return `${parseType(innerType)}[]`;
  }
